...
class board:
    ...
    def check_tie(self,game) -> bool:
        for row in game:
            if 0 in row:
                return False
        return True

    def check_win(self,game) -> bool:
        win = False
        if game[1][1] !=0:
    ...
    def computer_game(self,game,turn) -> list:
        # Checking diagonals
    ...
    def two_player(self,game,turn,w_move,error,player,start = False):
        if start:
        ...
        if player:
            ...
            if turn:
                ...
                game = self.computer_game(game,turn)
                if self.check_win(game):
                    ...
                    if not self.check_tie(game):
                        self.two_player(game,not turn,0,False,player)
                        return 0
                    else:
                        os.system('cls')
                        self.print_board(game)
        ...
        if not(not player and not turn):
            try:
                ...
                self.two_player(game,turn,0,True,player)
                return 0
            if row in [0,1,2] and column  in [0,1,2]:
                if game[row][column] != 0 :
                    os.system('cls')
                    self.print_board(game)
                    self.two_player(game,turn,1,False,player)
                    return 0
                else:
                    os.system('cls')
...
if __name__=="__main__":
    ...
    while True:
        ...
        if player == 1:
            game_obj.two_player(game,True,0,False,True,True)
            break
        elif player == 2:
            game_obj.two_player(game,randint(0,1),0,False,False,True)
            break
        else:
            print("Wrong input! Enter Again")